    ...
    await mongo().then(async (mongoose) => {
      try {
        ...
        if (interaction.options.getSubcommand() === "profile") {
          ...
          async function setGuildColor(clr, guildId, interaction) {
            await mongo().then(async (mongoose) => {
              try {
            ...
            const embed = new MessageEmbed()
              .setTitle("Guild Color")
              .setDescription(
                `You have succesfully set the guild color to \`${clr}\``
              )
          ...
          if (re.test(clr)) {
            if (clr.includes("#")) {
              ...
              const color = `#${clr}`;
              setGuildColor(color, guildId, interaction);
            }
          } else {
            interaction.reply({
          ...
          await mongo().then(async (mongoose) => {
            try {
              ...
              ctx.arc(100, 100, 100, 0, Math.PI * 2);
              ctx.fillStyle = results.guildColor;
              ctx.fill();
              ...
              const embed = new MessageEmbed()
                ...
                .setDescription(
                  `This server's color is \`${results.guildColor}\``
                )
                .setThumbnail("attachment://guildColor.png")
                .setColor(results.guildColor);
              interaction.reply({ embeds: [embed], files: [attachment] });
